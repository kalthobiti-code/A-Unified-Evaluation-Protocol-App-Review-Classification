...
DATASET = os.getenv("DATASET", "f-droid").lower()
DATA_PATH = os.getenv("DATA_PATH", "HLT_2841_Crashes_FeatureBugs.xlsx")

BASE_MODEL = os.getenv("BASE_MODEL", "meta-llama/Meta-Llama-3-8B")
USE_4BIT = True

PEFT_METHOD = os.getenv("PEFT_METHOD", "lora").lower()
...
OUTPUT_ROOT = Path("finetune_outputs")
...
class SparseTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if COMPOUND_SPARSE_ON and sparse_bias is not None:
            ...
            sparse_bias = sparse_bias.to(device)
...
def prepare_fold_datasets(fold: int):
    ...
    if COMPOUND_SPARSE_ON:
        keep_cols += ["sparse_flag", "keyword_positions"]
    ...
    if COMPOUND_SPARSE_ON:
        def add_sparse_cols(example, idx, dataset_source):
            return {
                "sparse_flag":       dataset_source[idx]["sparse_flag"],
                "keyword_positions": dataset_source[idx]["keyword_positions"]
            }
        pass
